...
class CapTPTestCase(unittest.TestCase, Crypto):
    ...
    @property
    def _next_import_object(self) -> captp_types.DescImportObject:
    ...
    @property
    def _next_answer(self) -> captp_types.DescAnswer:
        """ Returns the next answer position """
        position = self._next_answer_pos
        self._next_answer_pos += 1
        return captp_types.DescAnswer(position)

    def _expect_message_to(self, recipient: captp_types.DescExport, timeout=60):
    ...
    def _expect_promise_resolution(self, resolve_me_desc: captp_types.DescExport, timeout=60):
...
class CompleteCapTPTestCase(CapTPTestCase):
    ...
    def get_bootstrap_object(self, pipeline=False):
        ...
        bootstrap_op = captp_types.OpBootstrap(self._next_answer.position, self._next_import_object)
